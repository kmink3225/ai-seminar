# GEMINI.md — Gemini CLI 실행 규칙

> 이 파일은 Gemini CLI 가 **세션 시작 시 자동으로 읽는 진입점**이다.
...
- 긴 컨텍스트를 활용할 수 있어도 **필요한 가이드만 로드**한다 (토큰 낭비 방지).
- 도구 호출 결과가 길면 요약 후 원문은 인용만 남긴다.
- 파일 Write 전 **반드시 Read 선행**. 추측 금지.
