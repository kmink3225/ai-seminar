...
`/qa` 는 `../guides/qa.md` 의 Step 1~2 (쿼리 분해·병렬 Grep) 를 먼저 실행해 근거를 모은다. 결과를 사용자에게 1~2 줄로 보고한다.
...
```
예시:
"Step 1: references/md/ 에서 'tool use' 검색"
...
- 답변 후 관련 내부 문서 링크를 추가 제시
- 파일 생성·수정은 이 프로젝트 `/qa` 에서 발생하지 않음
...
3. **근거 인용** — 내부 문서 참조 시 `path:line` 형식.
4. **자동 생성물 직접 수정 금지** — `references/md/` 는 재생성으로만 갱신.
5. **이름 보존** — 기존 변수·함수·파일명 임의 변경 금지.

---

## Copilot CLI 특이사항

- 기본 제공되는 `suggest` / `explain` 형태의 한 줄 응답보다 **이 프로젝트에서는 위 Phase 프로토콜을 우선**한다.
- 쉘 명령 실행 전 destructive 여부(파일 삭제, force push, reset --hard 등) 를 사용자에게 확인한다.
- MCP / 외부 도구 호출은 신뢰된 로컬 리소스에 한해 사용한다.

---

## 라우팅 / 저장 규칙 / 프로젝트 정보

`../GUIDE.md` 가 canonical 소스다. 이 파일은 **Copilot 전용 Phase 프로토콜과 절대 규칙**만 담는다.
